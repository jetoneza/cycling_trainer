...
export let handleAction: (device: Device) => Promise<void>
export let handleToggleSpindown = (_: boolean) => {}
...
const getDeviceHRM = (device: Device) => {
  ...
  const { bpm, is_sensor_contact_supported, is_sensor_in_contact } = device
    .bleDevice.data as BasicObject
